...
We will see a quick setup to configure the ``transfersh`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``transfersh`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``transfersh`` formulas under ``/srv/salt/transfersh/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/transfersh/parameters/dns:domain/
    ln -s dns:domain /srv/salt/transfersh/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/transfersh/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/transfersh/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``transfersh`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``transfersh`` formulas under ``/srv/salt/transfersh/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/transfersh/parameters/roles
...
We create a configuration for the role ``transfersh/server`` in ``/srv/salt/transfersh/parameters/roles/transfersh/server.yaml``:
...
We create another configuration for the role ``transfersh/client`` in ``/srv/salt/transfersh/parameters/roles/transfersh/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles transfersh/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles transfersh/client
...
.. code-block:: console
    ...
    mv /srv/salt/transfersh/parameters/dns:domain/  '/srv/salt/transfersh/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-transfersh-as-json:
      test.nop:
        - name: {{ transfersh | json }}
...
Here is an example based on `template-formula/transfersh/config/file.sls`_:
...
.. code-block:: sls
    ...
    transfersh-config-file-file-managed:
      file.managed:
        - name: {{ transfersh.config }}
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='transfersh-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ transfersh.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``transfersh`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ transfersh['winner'] }}
...
.. _template-formula/transfersh/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/transfersh/config/file.sls
